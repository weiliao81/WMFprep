function [mask Header_Out] = indir_Automask(Data, OutputFile, Verb)
...
[Volume4D, ~, Header] = rp_readfile(Data); %Revised by Sun Jia-Wei 211002, change to use Read All.
    
%Get Mean Volume of Absolute 4D Volumes
MeanAbsVolume=mean(abs(Volume4D) , 4);

if Verb
    clip_fixed=ClipLevel(0.5, MeanAbsVolume);
...
if exist('OutputFile', 'var')==1 && ~isempty(OutputFile)
    ...
    rp_WriteNiftiImage(mask,Header_Out,OutputFile); %Revised by Sun Jia-Wei 211002, Output the mask.
end
...
clip_000=ClipLevel_partial(mask , mfrac , ...
    1   , icp , ...
    1   , jcp , ...
    1   , kcp);

clip_100=ClipLevel_partial(mask , mfrac , ...
    icm , nx , ...
    1   , jcp , ...
    1   , kcp);

clip_010=ClipLevel_partial(mask , mfrac , ...
    1   , icp , ...
    jcm , ny , ...
    1   , kcp);

clip_110=ClipLevel_partial(mask , mfrac , ...
    icm , nx , ...
    jcm , ny , ...
    1   , kcp);

clip_001=ClipLevel_partial(mask , mfrac , ...
    1   , icp , ...
    1   , jcp , ...
    kcm , nz);

clip_101=ClipLevel_partial(mask , mfrac , ...
    icm , nx , ...
    1   ,  jcp, ...
    kcm , nz);

clip_011=ClipLevel_partial(mask , mfrac , ...
    1   , icp , ...
    jcm , ny , ...
    kcm , nz);

clip_111=ClipLevel_partial(mask , mfrac , ...
    icm , nx , ...
    jcm , ny , ...
    kcm , nz);
%1
...
function clip=ClipLevel_partial( mask , mfrac ,...
    xa , xb ,...
    ya , yb ,...
    za , zb)

tmp_mask = mask(xa:xb , ya:yb , za:zb);
clip=ClipLevel(mfrac , tmp_mask);
...
function clip=ClipLevel(varargin)
%varargin{1}=mfac;
%varargin{2}=dataset;
