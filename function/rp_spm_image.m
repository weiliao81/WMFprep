function fg=rp_spm_image(op,varargin)
% image and header display
% FORMAT rp_spm_image
%_______________________________________________________________________
%
% rp_spm_image is an interactive facility that allows orthogonal sections
...
% John Ashburner
% $Id: rp_spm_image.m 3691 2010-01-20 17:08:30Z guillaume $


global st
global rp_spm_image_Parameters %YAN Chao-Gan 101010
...
if strcmp(op,'repos'),
    ...
    % spm_orthviews('MaxBB');
    %rp_spm_image('zoom_in');
    %rp_spm_image('update_info');
...
if strcmp(op,'addblobs'),
    ...
    for i=1:nblobs,
        ...
        set(st.blobber,'String','Remove Blobs','Callback','rp_spm_image(''rmblobs'');');
    end;
...
if strcmp(op,'rmblobs'),
    ...
    set(st.blobber,'String','Add Blobs','Callback','rp_spm_image(''addblobs'');');
...
if strcmp(op,'reorient'),
    ...
    rp_spm_image_Parameters.QCScore=QCScore;

    QCComment=get(st.QCCommentWidget, 'String');
    rp_spm_image_Parameters.QCComment=QCComment;
    
    mat = spm_matrix(st.B);
    rp_spm_image_Parameters.ReorientMat = mat;
    %YAN Chao-Gan 101010
%     if det(mat)<=0
%         spm('alert!','This will flip the images',mfilename,0,1);
%     end;
    P = rp_spm_image_Parameters.ReorientFileList;
...
%         rp_spm_image('init',st.vols{1}.fname);
%     end;
    close(fg);
    return;
end;
...
if strcmp(op,'DefineROI'),
    ...
    rp_spm_image_Parameters.pos=posTemp;
    close(fg);
    return;
end;
...
if ~isfield(rp_spm_image_Parameters,'ReorientFileList')
    rp_spm_image_Parameters.ReorientFileList={P};
end
if ~isfield(rp_spm_image_Parameters, 'QCScore')
    rp_spm_image_Parameters.QCScore=5;
end
if ~isfield(rp_spm_image_Parameters, 'QCComment')
    rp_spm_image_Parameters.QCComment='';
end
...
spm_orthviews('MaxBB');
st.callback = 'rp_spm_image(''shopos'');';
...
%-----------------------------------------------------------------------
uicontrol(fg,'Style','Frame','Position',[60 25 200 325].*WS,'DeleteFcn','rp_spm_image(''reset'');');
...
uicontrol(fg,'Style','edit','Callback','rp_spm_image(''repos'',1);','Position',[175 220 065 020].*WS,'String','0','ToolTipString','translate');
uicontrol(fg,'Style','edit','Callback','rp_spm_image(''repos'',2);','Position',[175 200 065 020].*WS,'String','0','ToolTipString','translate');
uicontrol(fg,'Style','edit','Callback','rp_spm_image(''repos'',3);','Position',[175 180 065 020].*WS,'String','0','ToolTipString','translate');
uicontrol(fg,'Style','edit','Callback','rp_spm_image(''repos'',4);','Position',[175 160 065 020].*WS,'String','0','ToolTipString','rotate');
uicontrol(fg,'Style','edit','Callback','rp_spm_image(''repos'',5);','Position',[175 140 065 020].*WS,'String','0','ToolTipString','rotate');
uicontrol(fg,'Style','edit','Callback','rp_spm_image(''repos'',6);','Position',[175 120 065 020].*WS,'String','0','ToolTipString','rotate');
uicontrol(fg,'Style','edit','Callback','rp_spm_image(''repos'',7);','Position',[175 100 065 020].*WS,'String','1','ToolTipString','zoom');
uicontrol(fg,'Style','edit','Callback','rp_spm_image(''repos'',8);','Position',[175  80 065 020].*WS,'String','1','ToolTipString','zoom');
uicontrol(fg,'Style','edit','Callback','rp_spm_image(''repos'',9);','Position',[175  60 065 020].*WS,'String','1','ToolTipString','zoom');

uicontrol(fg,'Style','Pushbutton','String','Reorient images','Callback','rp_spm_image(''reorient'');',...
...
uicontrol(fg,'Style','Pushbutton','String','Define ROI','Callback','rp_spm_image(''DefineROI'');',...
         'Position',[175 35 80 020].*WS,'ToolTipString','reset orientations of selected images');
% uicontrol(fg,'Style','Pushbutton','String','Reset...','Callback','rp_spm_image(''resetorient'')',...
...
st.mp = uicontrol(fg,'Style','edit', 'Position',[110 295 135 020].*WS,'String','','Callback','rp_spm_image(''setposmm'');','ToolTipString','move crosshairs to mm coordinates');
st.vp = uicontrol(fg,'Style','edit', 'Position',[110 275 135 020].*WS,'String','','Callback','rp_spm_image(''setposvx'');','ToolTipString','move crosshairs to voxel coordinates');
    ...
    switch i
        case 1
            st.QCScoreWidget{i} = uicontrol(fg, 'Style', 'Radiobutton', 'Position', [350 320-30*i 200 30].*WS,...
                'String', sprintf('1 - Very Poor'), 'Callback', @ QCScoreCallback, 'Value', rp_spm_image_Parameters.QCScore==1);
        case 2
            st.QCScoreWidget{i} = uicontrol(fg, 'Style', 'Radiobutton', 'Position', [350 320-30*i 200 30].*WS,...
                'String', sprintf('2 - Poor'), 'Callback', @ QCScoreCallback, 'Value', rp_spm_image_Parameters.QCScore==2);
        case 3
            st.QCScoreWidget{i} = uicontrol(fg, 'Style', 'Radiobutton', 'Position', [350 320-30*i 200 30].*WS,...
                'String', sprintf('3 - Fair'), 'Callback', @ QCScoreCallback, 'Value', rp_spm_image_Parameters.QCScore==3);
        case 4
            st.QCScoreWidget{i} = uicontrol(fg, 'Style', 'Radiobutton', 'Position', [350 320-30*i 200 30].*WS,...
                'String', sprintf('4 - Good'), 'Callback', @ QCScoreCallback, 'Value', rp_spm_image_Parameters.QCScore==4);
        case 5
            st.QCScoreWidget{i} = uicontrol(fg, 'Style', 'Radiobutton', 'Position', [350 320-30*i 200 30].*WS,...
                'String', sprintf('5 - Very Good'), 'Callback', @ QCScoreCallback, 'Value', rp_spm_image_Parameters.QCScore==5);
    end
end
...
st.QCCommentWidget = uicontrol(fg, 'Style', 'edit', 'Position',[310 30 270 100].*WS,...
    'Max', 10, 'HorizontalAlignment', 'left', 'BackgroundColor', 'White', 'String', rp_spm_image_Parameters.QCComment);
...
%     'Callback','rp_spm_image(''zoom_in'')','ToolTipString','zoom in by different amounts');
% c = 'if get(gco,''Value'')==1, spm_orthviews(''Space''), else, spm_orthviews(''Space'', 1);end;rp_spm_image(''zoom_in'')';
...
%     'String',char('Auto Window','Manual Window'),'Callback','rp_spm_image(''window'');','ToolTipString','range of voxel intensities displayed');
% % uicontrol(fg,'Style','pushbutton','Position',[315 35 125 20].*WS,...
% %   'String','Window','Callback','rp_spm_image(''window'');','ToolTipString','range of voxel intensities % displayed');
% st.blobber = uicontrol(fg,'Style','pushbutton','Position',[450 35 125 20].*WS,...
%     'String','Add Blobs','Callback','rp_spm_image(''addblobs'');','ToolTipString','superimpose activations');
end;
return;
